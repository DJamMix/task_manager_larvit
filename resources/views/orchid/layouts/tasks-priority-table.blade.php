    ...
    <div class="table-responsive">
        <table @class([
            ...
            @foreach($rows as $source)
                @php
                    $rowClass = '';
                    if (is_object($source) && method_exists($source, 'priorityRowClass')) {
                        $rowClass = $source->priorityRowClass();
                    } elseif (is_object($source) && isset($source->priority)) {
                        $rowClass = \App\CoreLayer\Enums\TaskPriorityEnum::tryFrom($source->priority)?->rowClass() ?? '';
                    }
                @endphp
                <tr class="{{ $rowClass }}">
                    @foreach($columns as $column)
